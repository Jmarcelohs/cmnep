...
// Margens da Base de Formatação oficial da Câmara: esquerda 3cm, direita
// 2cm, inferior 2cm. Superior mantida em 40mm (não os 3cm/30mm da regra) —
// o timbrado real tem uma faixa gráfica no topo mais alta que isso; 30mm
// cru encostaria o título nela (ver pedido anterior de "subir só um
// pouco"). Repetida em toda página de conteúdo, porque o timbrado real
// também se repete.
    ...
    node: (
      <div key="assinatura" className="mt-[16mm] flex flex-col items-center gap-[10mm]">
        <div className="text-center leading-normal">
          ...
          <p className="font-bold">{MESA_DIRETORA.presidente.cargo}</p>
          <p>{MESA_DIRETORA.bienio}</p>
        </div>
        <div className="flex w-full justify-around">
          <div className="text-center leading-normal">
            <p className="font-bold uppercase">{MESA_DIRETORA.vicePresidente.nome}</p>
            <p className="font-bold">{MESA_DIRETORA.vicePresidente.cargo}</p>
            <p>{MESA_DIRETORA.bienio}</p>
          </div>
          <div className="text-center leading-normal">
            <p className="font-bold uppercase">{MESA_DIRETORA.secretario.nome}</p>
            <p className="font-bold">{MESA_DIRETORA.secretario.cargo}</p>
            <p>{MESA_DIRETORA.bienio}</p>
          </div>
        </div>
      </div>
