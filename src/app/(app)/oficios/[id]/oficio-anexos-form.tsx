...
export function OficioAnexosForm({ oficioId, anexos }: { oficioId: string; anexos: Anexo[] }) {
  ...
  async function handleUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      for (const arquivo of Array.from(arquivos)) {
        const caminho = `${oficioId}/${crypto.randomUUID()}-${arquivo.name}`;
        ...
        const { error: erroInsert } = await supabase.from("oficios_anexos").insert({
          oficio_id: oficioId,
          caminho,
  ...
  async function handleExcluir(anexo: Anexo) {
    ...
    const supabase = createClient();
    await supabase.storage.from("oficios-anexos").remove([anexo.caminho]);
    await supabase.from("oficios_anexos").delete().eq("id", anexo.id);
    router.refresh();
  }
  ...
  return (
    <div className="rounded-lg border border-slate-200 bg-white p-4">
      ...
      <p className="mt-1 text-xs text-slate-500">
        PDF, Word (DOC/DOCX) e imagens (JPG/PNG/WEBP), até 15MB cada.
      </p>
